...
int SpinIndex = 0;
oxspine::SpineActor *actors = KLBNEWA(oxspine::SpineActor, 5); // 实例化对象数组
...
static IFactory::DEFCMD cmd[] = {
	...
	{"UI_CANVAS_CHANGERANIMATION",	UI_CANVAS_CHANGERANIMATION		},
	{0, 0}
};
...
bool 
CKLBUICanvas::addAsset(const char* asset, const char *atlasStr, const char*jsonStr)
{
	u32 handle;
	...
	if(pAsset && (pAsset->getAssetType() == ASSET_IMAGE)) {
		...
		//reset coord
		   ...
		   //AddSpine(pImgAsset, atlasStr, jsonStr);
		if (atlasStr != NULL && jsonStr != NULL) {
			oxspine::SpineActor* actor = new oxspine::SpineActor();
			actor->AddSpine(pImgAsset, atlasStr, jsonStr);
			actors[SpinIndex] = *actor;
			SpinIndex++;
		}
...
CKLBUICanvas::commandUI(CLuaState& lua, int argc, int cmd)
{
	...
	case UI_CANVAS_ADDRESOURCE:
		if (argc == 3){
			addAsset(lua.getString(3), NULL, NULL);	
		}
		if (argc == 5)
		{
			addAsset(lua.getString(3), lua.getString(4), lua.getString(5));
		}
		ret = 1;
		result = true;
		break;
	...
	case UI_CANVAS_SECTIONCOLOR:
		...
		break;
	case UI_CANVAS_CHANGERANIMATION: //改变动画
		...
		if (argc == 5) {
			...
			int id = lua.getInt(3);
			SpineActor *actor = &actors[id];
			actor->changerAnimation(lua.getString(4),lua.getBool(5));
			//animationState->setAnimation(0, lua.getString(3), true);
		}
